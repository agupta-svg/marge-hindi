...
use Getopt::Std;
#-------------------------------------------------------------------------------------
use vars qw($opt_a $opt_b $opt_c $opt_d $opt_e $opt_f $opt_g $opt_h $opt_H $opt_m $opt_n $opt_p $opt_s $opt_t $opt_l $opt_v $opt_w $opt_2 $opt_u $opt_x $opt_U $opt_3 $opt_M $opt_z);
...
my $usage="$0\n         [-a (evaluate all systems)] 
         ...
         <ROUGE-eval-config-file> [<systemID>]
";
getopts('ahHb:c:de:f:l:gmMn:p:st:r:2:3:w:uUvxz:');
my $systemID;
...
sub createNGram {
  ...
  for($i=0;$i<=$#mx_tokens;$i++) {
    ...
    if(defined($opt_g)) {
      use Text::HindiStem;
      my $stem = new Text::HindiStem->new();
      $stem = $mx_tokens[$i];
      if (length($stem) > 3) {
        my $stemmed_word = $stemmer->stem($stem);
        push(@m_tokens, $stemmed_word);
      } else {
        push(@m_tokens, $mx_tokens[$i]);
        }
    }
...
sub createBE {
  ...
  for($i=0;$i<=$#{$BEList};$i++) {
    ...
    if(defined($opt_g)){
      ...
      if (length($stemH) > 3) {
        my $stemmerH = Text::HindiStem->new();
        $stemH = $stemmerH->stem($stemH);
        }
  
      if ($stemM ne "NIL" && length($stemM) > 3) {
        my $stemmerM = Text::HindiStem->new();
        $stemM = $stemmerM->stem($stemM);
         }
    }
    if($BEMode eq "H"&&
      $stemM eq "nil") {
...
sub tokenizeText {
  ...
  for($i=0;$i<=$#mx_tokens;$i++) {
    ...
    if(defined($opt_g)) {
      ...
      if (length($stem) > 3) {
        my $stemmed_word = $stemmer->stem($stem);
        push(@{$tokenizedText}, $stemmed_word);
      } else {
        push(@{$tokenizedText}, $mx_tokens[$i]);
        }
    }
	...
	if($mx_tokens[$i]=~/^[a-z0-9\$]/o) {
	  if(defined($opt_m)) {
        ...
        if(defined($opt_g)) {
          ...
          if (length($stem) > 3) {
            my $stemmed_word = $stemmer->stem($stem);
            push(@{$tokenizedText->[$t]} $stemmed_word);
            } else {
              push(@{$tokenizedText->[$t]}, $mx_tokens[$i]);
               }
    }
